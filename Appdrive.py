...
@st.cache_resource
def download_models_from_hf(url="https://huggingface.co/Jkkeer/univ-recommender-models/resolve/main/models.zip", base_dir="modelsC"):
...
def load_specialization_models():
    ...
    major_path = os.path.join(model_dir, "models", "major_models_compressed")
    try:
        models = {
        ...
        st.error(f"❌ Error loading specialization models:")
        st.exception(e)
        return None
...
def load_university_models():
    model_dir = download_models_from_hf()
    university_path = os.path.join(model_dir, "models", "university_models_compressed")
    try:
        models = {
        ...
        st.error(f"❌ Error loading university models:")
        st.exception(e)
        return None
